...
GeneralScene::GeneralScene(Mouse* attached_mouse, Scene* prev_scene, PageUnitEx* pageunit_list_head)
...
void GeneralScene::waitInput(bool isKeyboardInputOn, bool isMouseInputOn) throw() {
	...
	if (isKeyboardInputOn) {
		...
		if (KEY_DOWN(VK_RETURN) && unit_current_ != NULL)findLink(unit_current_->getPageUnit().text);
	}
	if (!((mouse_msg_ = (*attached_mouse_).hoverAndClick(pageunit_list_head_)).empty()) && isMouseInputOn) {
		findLink(mouse_msg_);
	}
}
