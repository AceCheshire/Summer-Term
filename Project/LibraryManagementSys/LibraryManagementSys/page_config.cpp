...
void PageUnitEx::append(PageUnit& pageunit_list_end) {
	...
	else next_unit_->append(pageunit_list_end);
}

PageUnitEx* PageUnitEx::findByText(std::string text) {
	if (this_unit_.text == text)return this;
	else if (next_unit_ != NULL)return next_unit_->findByText(text);
	else return NULL;
}
...
void Page::returnDefault() {
	if (!SetConsoleTextAttribute(handle_page_output_, GROUND_DEFAULT)) throw COLOR_ERROR;
}

bool Page::pointPaint(PageUnit request_unit) throw() {
	try {
		if (!SetConsoleTextAttribute(handle_page_output_, request_unit.color)) throw COLOR_ERROR;
		if (!SetConsoleCursorPosition(handle_page_output_, request_unit.pos)) throw COORD_ERROR;
...
bool Page::pointPaint(COORD request_pos, WORD request_color) throw() {
	try {
		if (!SetConsoleTextAttribute(handle_page_output_, request_color)) throw COLOR_ERROR;
		if (!SetConsoleCursorPosition(handle_page_output_, request_pos)) throw COORD_ERROR;
	}
	catch (BasicError err_num) {
		SetConsoleTextAttribute(handle_page_output_, FOREGROUND_RED);
		cout << endl;
...
bool Page::pointPaint(COORD request_pos, WORD request_color, string request_string) throw() {
	try {
		if (!SetConsoleTextAttribute(handle_page_output_, request_color)) throw COLOR_ERROR;
		if (!SetConsoleCursorPosition(handle_page_output_, request_pos)) throw COORD_ERROR;
...
bool Page::pointPaint(short i, short j, WORD request_color, string request_string) throw() {
	COORD request_pos = { i, j };
	try {
		if (!SetConsoleTextAttribute(handle_page_output_, request_color)) throw COLOR_ERROR;
		if (!SetConsoleCursorPosition(handle_page_output_, request_pos)) throw COORD_ERROR;
