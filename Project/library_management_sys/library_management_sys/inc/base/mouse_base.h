...
class Mouse {
 ...
 protected:
  ...
  // Assign read_str_ with std::string that mouse is hovering on. Its argument
  // is for internal popLastStr to match with.
  void readCursorChars(PageUnitEx& source_text);
  // Re-paints the std::string mouse used to hover on if mouse position has
  // changed. Argument is the pageunit_list_head_ of a Scene.
  ...
  // Equivalent to point_mouse_pos_.
