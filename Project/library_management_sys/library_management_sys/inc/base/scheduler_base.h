...
// It means how many std::strings appear on pages
// can be Link.
constexpr int kMaxLinkNum = 256;
}  // namespace scheduler

// Coordinate different scenes like finite - state_ machine.
// Example:
//    Mouse test_mouse;
//    Scheduler test_scheduler(test_mouse, 0);
class Scheduler {
 public:
  // Constructor. The second argument should be the serial of the scene client
  // wanna display first.
  Scheduler(Mouse& attached_mouse, unsigned short init_scene);
  ...
  bool generalSwitch(unsigned short scene_serial, unsigned short check_mode);
 ...
 protected:
  // Answers to keyboard and mouse input. Arguments are input states (mostly
  // true) Pay attention that keyboard input may be overridden by mouse input.
  // Example:
  ...
  bool findLink(const std::wstring& msg);
  ...
  // Used to record the input state_, which can be divided into
